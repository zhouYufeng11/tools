class Print {
    ...
    createElement() {   // 创建自定义元素表格
       ...
       const total = Math.ceil(rowsLen / step) || 1;
       let page = 1;

       // 首页
       const oElem = this.renderElement( title, headers, rows.slice(count, count += 28), page, total );
...
function customBaseMessageTemplate( data ) {
    ...
    <div class="print-base-flex">
        <div>住院天数: ${ data.dayCount }</div>
        <div>预缴费用: ${ data.mealRechargeAmount }</div>
        <div>总费用: ${ data.mealAmount }</div>
    </div>`;
